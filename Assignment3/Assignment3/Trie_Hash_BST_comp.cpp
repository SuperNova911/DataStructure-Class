...
int main()
{
	...
	for (N = 100000; N <= 10000000; N *= 10)
	{
		...
		for (int index = 0; index < N; index++)
		{
			novelTrie.Search(randomKey[index].c_str());
		}
		elapsed_time = getdtime() - oldtime;
		printf("N=%d, Trie search, elapsed_time: %lf sec\n", N, elapsed_time);
	}
